...
The home page plays two recorded flights, chosen with the picker. Their telemetry was measured during the same flight as the imagery, so attitude, speed, altitude and position are real readings. The page names the dataset, its license, and any value the converter resolved instead of recording. The silent local videos pause with the HUD; reduced-motion preferences start them paused. See [recorded flight provenance](public/replay/README.md) for sources, processing and hashes, and [replay recorded flights](../docs/replay.md) for the converters. The media is excluded from the npm package.
